...
export function pickStrengthWorkout(userSettings: UserSettings): Exercise[] {
  ...
  const filteredExercises = _.filter(exercises, e => {
    ...
    const equipmentMatches =
      !e.equipment || _.every(e.equipment, eq => equipment.includes(eq));
  ...
  const upperBodyExercises = _.filter(
    filteredExercises,
    e => e.category === 'upper'
  );
  const lowerBodyExercises = _.filter(
    filteredExercises,
    e => e.category === 'lower'
  );
...
export function getFilteredExercisesByCategory(
  difficulty: Difficulty,
  equipment: string[] = []
) {
  const filteredExercises = _.filter(exercises, exercise => {
    ...
    const equipmentMatches =
      !exercise.equipment ||
      _.every(exercise.equipment, eq => equipment.includes(eq));
...
export function countExcludedExercises(
  ...
  return _.filter(excludedExercises, name =>
    _.includes(availableExerciseNames, name)
  ).length;
}
